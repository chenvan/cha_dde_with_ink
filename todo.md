# 2022/7/15
- [] 程序出现"Malformed CString error", 可能是导致程序 crash 的原因

- [] 15, 16号加料后柜的左极限信号捕捉困难, 怀疑是位置没摆好, 在车启动的附近, 容易冲过去, 需要到现场查看

- [] 回潮后柜分配车有时候要在布料车来回两遍后才会挪动一下, 不知道是信号采集的问题, 还是其他原因导致, 因此暂时无法对它实行入柜检测

- [] 入柜监控要知道入柜方式, 因为车的停下布料的时间会随着入柜方式的不同而改变

- [] 出柜检测半眼会出现多次警告, 可能是 fetch 半眼状态的时间过长, wbAccu 已经变换好几次了

- [] 96加料出柜有时候无法读取 total 这个存柜总量值

- [] 出柜号是空的时候, cabinetNr 是否是 0 值
